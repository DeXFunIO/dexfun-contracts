...
describe("Oracle.RealtimeFeeds", () => {
  ...
  it("realtime feed validations", async () => {
    ...
    await dataStore.setBytes32(
      keys.realtimeFeedIdKey(wnt.address),
      hashString("WNT")
    );
    ...
    await dataStore.setBytes32(
      keys.realtimeFeedIdKey(wbtc.address),
      hashString("WBTC")
    );
    ...
    await expect(
      ...
      .to.be.revertedWithCustomError(
        errorsContract,
        "EmptyRealtimeFeedMultiplier"
      )
      .withArgs(wnt.address);

    await dataStore.setUint(
      keys.realtimeFeedMultiplierKey(wnt.address),
      expandDecimals(1, 34)
    );
    ...
    await dataStore.setAddress(
      keys.priceFeedKey(wnt.address),
      wntPriceFeed.address
    );
    await dataStore.setUint(
      keys.priceFeedMultiplierKey(wnt.address),
      expandDecimals(1, 42)
    );
    await dataStore.setUint(
  ...
  it("sets prices with realtime feeds", async () => {
    ...
    await dataStore.setUint(
      keys.realtimeFeedMultiplierKey(wbtc.address),
      expandDecimals(1, 44)
    );
    ...
    expect((await oracle.getPrimaryPrice(wnt.address)).min).eq(
      expandDecimals(5000, 12)
    );
    expect((await oracle.getPrimaryPrice(wnt.address)).max).eq(
      expandDecimals(5002, 12)
    );

    expect((await oracle.getPrimaryPrice(wbtc.address)).min).eq(
      expandDecimals(75_000, 22)
    );
    expect((await oracle.getPrimaryPrice(wbtc.address)).max).eq(
      expandDecimals(75_020, 22)
    );
  });
  ...
  it("sets prices with regular and realtime feeds", async () => {
    ...
    await oracle.setPrices(dataStore.address, eventEmitter.address, {
      ...
      signerInfo,
      tokens: [wbtc.address],
      compactedMinOracleBlockNumbers: getCompactedOracleBlockNumbers([
        block.number,
      ]),
      compactedMaxOracleBlockNumbers: getCompactedOracleBlockNumbers([
        block.number,
      ]),
      compactedOracleTimestamps: getCompactedOracleTimestamps([
        block.timestamp,
      ]),
