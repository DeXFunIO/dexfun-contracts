...
describe("MarketStoreUtils", () => {
  ...
  it("get, set, remove", async () => {
    ...
    const setItem = async (dataStore, key, sampleItem) => {
      return await marketStoreUtilsTest.setMarket(
        dataStore.address,
        key,
        marketType,
        sampleItem
    ...
    const removeItem = async (dataStore, itemKey) => {
      return await marketStoreUtilsTest.removeMarket(
        dataStore.address,
        itemKey
      );
    };
    ...
    expect(await getItemKeys(dataStore, 0, 11)).deep.equal(
      initialItemKeys.concat(itemKey)
    );
