...
describe("ExchangeRouter", () => {
  ...
  async function decodeRevertReason(txHash) {
  ...
  it("createDeposit", async () => {
    ...
    await usdc
      .connect(user0)
      .approve(router.address, expandDecimals(50 * 1000, 6));

    await wnt.mint(user0.address, expandDecimals(50 * 1000, 18));
    // try {
    //   const result = await exchangeRouter
    //     .connect(user0)
    //     .sendWnt(depositVault.address, expandDecimals(11, 18));
    //   await result.wait();
    //   console.log("sendWnt transaction succeeded");
    // } catch (error) {
    //   console.error("sendWnt transaction failed:", error);
    //   // const revertReason = await decodeRevertReason(error.transactionHash);
    //   // console.error("Revert reason for sendWnt:", revertReason);
    //   return;
    // }
    // try {
    //   const tx2 = await exchangeRouter
    //     .connect(user0)
    //     .sendTokens(
    //       usdc.address,
    ...
    //     );
    //   await tx2.wait();
    //   console.log("sendTokens transaction succeeded");
    // } catch (error) {
    //   console.error("sendTokens transaction failed:", error);
    //   const revertReason = await decodeRevertReason(error.transactionHash);
    //   console.error("Revert reason for sendTokens:", revertReason);
    //   return;
    // }

    // try {
    //   const tx3 = await exchangeRouter.connect(user0).createDeposit({
    ...
    //   console.log("createDeposit transaction succeeded");
    // } catch (error) {
    //   console.error("createDeposit transaction failed:", error);
    //   const revertReason = await decodeRevertReason(error.transactionHash);
    //   console.error("Revert reason for createDeposit:", revertReason);
    //   return;
    // }
    console.log(ethUsdMarket.marketToken);
    const tx = await exchangeRouter.connect(user0).multicall(
      [
        exchangeRouter.interface.encodeFunctionData("sendWnt", [
          depositVault.address,
          expandDecimals(11, 18),
        ]),
    ...
    console.log("key", dataStore.address, depositKeys[0]);
  ...
  it("createOrder", async () => {
    ...
    expect(order.numbers.decreasePositionSwapType).eq(
      DecreasePositionSwapType.SwapCollateralTokenToPnlToken
    );
    expect(order.numbers.sizeDeltaUsd).eq(decimalToFloat(1000));
    expect(order.numbers.initialCollateralDeltaAmount).eq(
  ...
  it("createWithdrawal", async () => {
    ...
    const marketToken = await contractAt(
      "MarketToken",
      ethUsdMarket.marketToken
    );
    await marketToken
      .connect(user0)
      .approve(router.address, expandDecimals(50 * 1000, 18));
    console.log("after market token");
    ...
    console.log("withdrawal key:", withdrawalKeys);
