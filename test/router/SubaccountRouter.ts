...
describe("SubaccountRouter", () => {
  ...
  it("setMaxAllowedSubaccountActionCount", async () => {
    expect(
      await dataStore.getUint(
        keys.maxAllowedSubaccountActionCountKey(
          user0.address,
          user1.address,
  ...
  it("setSubaccountAutoTopUpAmount", async () => {
    expect(
      await dataStore.getUint(
        keys.subaccountAutoTopUpAmountKey(user0.address, user1.address)
      )
    ).eq(0);

    await subaccountRouter
      .connect(user0)
      .setSubaccountAutoTopUpAmount(user1.address, 101);
  ...
  it("MarketIncrease order", async () => {
    ...
    await subaccountRouter
      ...
      .multicall(
        [
          subaccountRouter.interface.encodeFunctionData("sendNativeToken", [
            subaccount.address,
            expandDecimals(1, 18),
          ]),
          subaccountRouter.interface.encodeFunctionData("addSubaccount", [
            subaccount.address,
          ]),
          subaccountRouter.interface.encodeFunctionData(
            "setMaxAllowedSubaccountActionCount",
            [subaccount.address, keys.SUBACCOUNT_ORDER_ACTION, 20]
          ),
          subaccountRouter.interface.encodeFunctionData(
            "setSubaccountAutoTopUpAmount",
            [subaccount.address, expandDecimals(2, 17)]
          ),
        ],
    ...
    expect(
      await dataStore.getUint(
        keys.subaccountAutoTopUpAmountKey(user0.address, subaccount.address)
      )
    ).eq(expandDecimals(2, 17));
    ...
    await expect(
      subaccountRouter
        .connect(subaccount)
        .createOrder(user0.address, { ...params })
    )
    ...
    await expect(
      subaccountRouter.connect(subaccount).createOrder(user0.address, params)
    )
    ...
    await subaccountRouter.connect(subaccount).multicall(
      [
        subaccountRouter.interface.encodeFunctionData("sendWnt", [
          orderVault.address,
          expandDecimals(1, 17),
        ]),
    ...
    await subaccountRouter
      ...
      .multicall(
        [
          ...
          subaccountRouter.interface.encodeFunctionData(
            "setSubaccountAutoTopUpAmount",
            [subaccount.address, expandDecimals(1, 17)]
          ),
        ],
        { value: expandDecimals(1, 18) }
      );
  ...
  it("cancel order", async () => {
    ...
    await expect(
      subaccountRouter.connect(subaccount).cancelOrder(orderKeys[0])
    ).to.be.revertedWithCustomError(errorsContract, "SubaccountNotAuthorized");
  });
});
