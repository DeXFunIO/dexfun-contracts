...
describe("Exchange.VirtualSwapPriceImpact", () => {
  ...
  beforeEach(async () => {
    ...
    await dataStore.setBytes32(
      keys.virtualMarketIdKey(ethUsdcMarket.marketToken),
      ethUsdVirtualMarketId
    );
    await dataStore.setBytes32(
      keys.virtualMarketIdKey(ethUsdtMarket.marketToken),
      ethUsdVirtualMarketId
    );
  ...
  it("uses virtual inventory for price impact", async () => {
    ...
    await dataStore.setUint(
      keys.swapImpactFactorKey(ethUsdcMarket.marketToken, true),
      decimalToFloat(5, 9)
    );
    await dataStore.setUint(
      keys.swapImpactFactorKey(ethUsdcMarket.marketToken, false),
      decimalToFloat(1, 8)
    );
    await dataStore.setUint(
      keys.swapImpactExponentFactorKey(ethUsdcMarket.marketToken),
      decimalToFloat(2, 0)
    );

    await dataStore.setUint(
      keys.swapImpactFactorKey(ethUsdtMarket.marketToken, true),
      decimalToFloat(5, 9)
    );
    await dataStore.setUint(
      keys.swapImpactFactorKey(ethUsdtMarket.marketToken, false),
      decimalToFloat(1, 8)
    );
    await dataStore.setUint(
      keys.swapImpactExponentFactorKey(ethUsdtMarket.marketToken),
      decimalToFloat(2, 0)
    );
    ...
    await usingResult(
      ...
      async ({ executeResult }) => {
        const { logs } = executeResult;
        const swapInfo = getEventData(logs, "SwapInfo");
        expect(swapInfo.priceImpactUsd).eq(
          "-299599999999999993940174820850000"
        ); // -299.6
      }
    );
  });
