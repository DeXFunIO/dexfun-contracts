...
describe("Exchange.Deposit", () => {
  ...
  it("createDeposit validations", async () => {
    ...
    const _createDepositFeatureDisabledKey =
      keys.createDepositFeatureDisabledKey(depositHandler.address);
    ...
    await expect(
      createDeposit(fixture, {
        ...params,
        market: {
          marketToken: user1.address,
          longToken: wnt.address,
          shortToken: usdc.address,
        },
      })
    ).to.be.revertedWithCustomError(errorsContract, "EmptyMarket");

    const _isMarketDisabledKey = keys.isMarketDisabledKey(
      ethUsdMarket.marketToken
    );
    ...
    await expect(createDeposit(fixture, params)).to.be.revertedWithCustomError(
      errorsContract,
      "EmptyDepositAmounts"
    );
    ...
    await expect(
      ...
      .withArgs("2000000014000000", "3000");
  });
  ...
  it("createDeposit", async () => {
    ...
    expect(deposit.numbers.initialShortTokenAmount).eq(
      expandDecimals(10 * 5000, 6)
    );
  ...
  it("cancelDeposit", async () => {
    ...
    const _cancelDepositFeatureDisabledKey =
      keys.cancelDepositFeatureDisabledKey(depositHandler.address);
  ...
  it("executeDeposit validations", async () => {
    await expect(
      depositHandler.connect(user1).executeDeposit(HashZero, {
        signerInfo: 0,
        tokens: [],
      ...
      .to.be.revertedWithCustomError(errorsContract, "Unauthorized")
      .withArgs(user1.address, "ORDER_KEEPER");
    ...
    const _executeDepositFeatureDisabledKey =
      keys.executeDepositFeatureDisabledKey(depositHandler.address);
    ...
    await expect(
      ...
      errorsContract,
      "OracleBlockNumberNotWithinRange"
    );
    ...
    await dataStore.setUint(
      keys.maxPoolAmountKey(ethUsdMarket.marketToken, wnt.address),
      expandDecimals(1, 18)
    );
    ...
    await dataStore.setUint(
      keys.minMarketTokensForFirstDeposit(ethUsdMarket.marketToken),
      expandDecimals(1, 18)
    );
  ...
  it("_executeDeposit", async () => {
    const depositStoreUtilsTest = await deployContract(
  ...
  it("executeDeposit, spot only market", async () => {
    const revertingCallbackReceiver = await deployContract(
    ...
    expect(
      await getBalanceOf(ethUsdSpotOnlyMarket.marketToken, user0.address)
    ).eq("50000000000000000000000"); // 50,000
    expect(await wnt.balanceOf(depositVault.address)).eq(0);
    expect(await usdc.balanceOf(depositVault.address)).eq(0);

    expect(await wnt.balanceOf(ethUsdSpotOnlyMarket.marketToken)).eq(
    ...
    expect(
      await getBalanceOf(ethUsdSpotOnlyMarket.marketToken, user0.address)
    ).eq("75000000000000000000000"); // 75,000
    expect(await wnt.balanceOf(depositVault.address)).eq(0);
    expect(await usdc.balanceOf(depositVault.address)).eq(0);
    ...
    expect(await usdc.balanceOf(ethUsdSpotOnlyMarket.marketToken)).eq(
  ...
  it("price impact", async () => {
    ...
    await dataStore.setUint(
      keys.swapImpactFactorKey(ethUsdMarket.marketToken, true),
      decimalToFloat(1, 8)
    );
    await dataStore.setUint(
      keys.swapImpactFactorKey(ethUsdMarket.marketToken, false),
      decimalToFloat(1, 8)
    );
    await dataStore.setUint(
      keys.swapImpactExponentFactorKey(ethUsdMarket.marketToken),
      decimalToFloat(2, 0)
    );
    ...
    expect(await getBalanceOf(ethUsdMarket.marketToken, user0.address)).eq(
      "49975000000000000000000"
    );
    expect(await wnt.balanceOf(depositVault.address)).eq(0);
    expect(await usdc.balanceOf(depositVault.address)).eq(0);

    expect(await wnt.balanceOf(ethUsdMarket.marketToken)).eq(
      expandDecimals(10, 18)
    );
    expect(await usdc.balanceOf(ethUsdMarket.marketToken)).eq(0);
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq("9995000000000000000");
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(0);
    expect(
      await getSwapImpactPoolAmount(
        dataStore,
        ethUsdMarket.marketToken,
        wnt.address
      )
    ...
    expect(await getBalanceOf(ethUsdMarket.marketToken, user1.address)).eq(
      "49999975006249999995000"
    ); // 49999.975006249999995
  });
  ...
  it("positive and negative price impact", async () => {
    ...
    expect(await getBalanceOf(ethUsdMarket.marketToken, user1.address)).eq(
      "49987487503124999995000"
    ); // 49987.487503124999995
  });
  ...
  it("price impact split over multiple orders", async () => {
    ...
    expect(await getBalanceOf(ethUsdMarket.marketToken, user2.address)).eq(
      "50024996875000000000000"
    ); // 50024.996875

    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq(expandDecimals(10, 18));
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(50 * 1000, 6));
    expect(
      await getSwapImpactPoolAmount(
        dataStore,
        ethUsdMarket.marketToken,
  ...
  it("!isSameSideRebalance, net negative price impact", async () => {
    ...
    expect(await getMarketTokenPrice(fixture)).eq(expandDecimals(1, 30));
    expect(await getBalanceOf(ethUsdMarket.marketToken, user1.address)).eq(
      "99987462496000000000000"
    ); // 99987.462496

    expect(await wnt.balanceOf(ethUsdMarket.marketToken)).eq(
      expandDecimals(10, 18)
    );
    expect(await usdc.balanceOf(ethUsdMarket.marketToken)).eq(
  ...
  it("price impact, fees", async () => {
    // 0.05%: 0.0005
    await dataStore.setUint(
      keys.swapFeeFactorKey(ethUsdMarket.marketToken, true),
      decimalToFloat(5, 4)
    );
    await dataStore.setUint(
      keys.swapFeeFactorKey(ethUsdMarket.marketToken, false),
      decimalToFloat(5, 4)
    );
    // 30%
    await dataStore.setUint(
    ...
    expect(await getMarketTokenPrice(fixture)).eq(
      "1000350350350350350350350350350"
    ); // 1.00035035035

    expect(await getBalanceOf(ethUsdMarket.marketToken, user0.address)).eq(
      "49950000000000000000000"
    ); // 49950
    expect(await wnt.balanceOf(depositVault.address)).eq(0);
    expect(await usdc.balanceOf(depositVault.address)).eq(0);
    expect(
      await getClaimableFeeAmount(
        dataStore,
        ethUsdMarket.marketToken,
        wnt.address
      )
    ).eq("1500000000000000"); // 0.0015 ETH, 7.5 USD
    expect(
      await getClaimableFeeAmount(
        dataStore,
        ethUsdMarket.marketToken,
    ...
    expect(await getMarketTokenPrice(fixture)).eq(
      "1000525446705012120185491696460"
    ); // ~1.00052

    expect(await getBalanceOf(ethUsdMarket.marketToken, user1.address)).eq(
      "49944998007168690894085"
    ); // 49944.998
    expect(await wnt.balanceOf(ethUsdMarket.marketToken)).eq(
      "10000000000000000000"
    ); // 10 ETH
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq("49967503750"); // 49967.50375 USDC
    expect(
      await getSwapImpactPoolAmount(
        dataStore,
        ethUsdMarket.marketToken,
  ...
  it("single token market", async () => {
    ...
    expect(await getSupplyOf(ethUsdSingleTokenMarket.marketToken)).eq(
