...
describe("Exchange.DecreasePosition.InsolventClose", () => {
  ...
  it("funding fees > collateral", async () => {
    ...
    const positionKey1 = getPositionKey(
      user0.address,
      ethUsdMarket.marketToken,
      wnt.address,
      false
    ...
    await usingResult(
      ...
      (positionInfo) => {
        ...
        expect(positionInfo.basePnlUsd).eq(0);
        expect(positionInfo.fees.funding.fundingFeeAmount).closeTo(
          "106036363636",
          "10"
        ); // 106,036.363636
        expect(positionInfo.fees.funding.claimableLongTokenAmount).eq(0);
    ...
    await usingResult(
      ...
      (positionInfo) => {
        ...
        expect(positionInfo.fees.funding.fundingFeeAmount).eq(0);
        expect(positionInfo.fees.funding.claimableLongTokenAmount).eq(0);
        expect(positionInfo.fees.funding.claimableShortTokenAmount).closeTo(
          "106036363636",
          "10"
        ); // 106,036.363636
      }
    ...
    await usingResult(
      ...
      ([marketTokenPrice, poolValueInfo]) => {
        ...
        expect(poolValueInfo.poolValue).eq(
          "6019280000000000000000000000000000000"
        );
      }
    );
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq("999200000001"); // 999,200.000001
    ...
    await usingResult(
      ...
      ([marketTokenPrice, poolValueInfo]) => {
        ...
        expect(poolValueInfo.poolValue).eq(
          "5999200000001000000000000000000000000"
        );
      }
    );
    ...
    await usingResult(
      ...
      ([marketTokenPrice, poolValueInfo]) => {
        ...
        expect(poolValueInfo.poolValue).eq(
          "6019280000001000000000000000000000000"
        );
      }
    );
  });
  ...
  it("funding fees > collateral, unable to swap", async () => {
    ...
    await dataStore.setUint(
      keys.reserveFactorKey(ethUsdMarket.marketToken, false),
      decimalToFloat(2, 2)
    );
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq("999840637450199203188"); // 999.840637450199203188 ETH
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(1_000_000, 6));
    ...
    await usingResult(
      ...
      ([marketTokenPrice, poolValueInfo]) => {
        ...
        expect(poolValueInfo.poolValue).eq(
          "5999203187250996015940000000000000000"
        );
      }
    );
    ...
    await usingResult(
      ...
      ([marketTokenPrice, poolValueInfo]) => {
        ...
        expect(poolValueInfo.poolValue).eq(
          "6019280000000000000003760000000000000"
        );
      }
    );
  });
