...
describe("Exchange.DecreasePosition", () => {
  ...
  it("negative price impact, positive pnl", async () => {
    ...
    await usingResult(
      ...
      ([marketTokenPrice, poolValueInfo]) => {
        ...
        expect(poolValueInfo.poolValue).eq(
          "6020000000000000000000000000000000000"
        );
      }
    );
    ...
    ).eq("40796812749003984"); // 0.040796812749003984 ETH
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq("999992063745019920319"); // 999.992063745019920319
    // 4 USD was paid from the position's collateral for price impact
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(1_000_004, 6));
    ...
    await usingResult(
      ...
      ([marketTokenPrice, poolValueInfo]) => {
        ...
        expect(poolValueInfo.poolValue).eq(
          "5999920334661354581680000000000000000"
        );
      }
    );
    ...
    await usingResult(
      ...
      ([marketTokenPrice, poolValueInfo]) => {
        ...
        expect(poolValueInfo.poolValue).eq(
          "6020000000000000000006720000000000000"
        );
      }
    );
  });
