...
describe("Exchange.PositionPriceImpact.SyntheticMarket", () => {
  ...
  beforeEach(async () => {
    ...
    await handleDeposit(fixture, {
      ...
      execute: {
        ...getExecuteParams(fixture, {
          prices: [prices.sol, prices.wnt, prices.usdc],
        }),
      },
    });
  });
  ...
  it("price impact", async () => {
    await dataStore.setUint(
      keys.positionImpactFactorKey(solUsdMarket.marketToken, true),
      decimalToFloat(5, 9)
    );
    await dataStore.setUint(
      keys.positionImpactFactorKey(solUsdMarket.marketToken, false),
      decimalToFloat(1, 8)
    );
    await dataStore.setUint(
      keys.positionImpactExponentFactorKey(solUsdMarket.marketToken),
      decimalToFloat(2, 0)
    );
    ...
    expect(
      await dataStore.getUint(
        keys.positionImpactPoolAmountKey(solUsdMarket.marketToken)
      )
    ).eq(0);
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        ...
        afterExecution: ({ logs }) => {
          ...
          expect(positionIncreaseEvent.executionPrice).eq(
            "50100200400801603206412"
          ); // 50.1002004008
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        ...
        afterExecution: ({ logs }) => {
          ...
          expect(positionDecreaseEvent.executionPrice).eq(
            "50050100200400801602278"
          ); // 50.05
