...
describe("Exchange.SwapOrder", () => {
  ...
  beforeEach(async () => {
    ...
    ({ dataStore, ethUsdMarket, ethUsdSpotOnlyMarket, wnt, usdc } =
      fixture.contracts);
  ...
  it("price impact", async () => {
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        ...
        afterExecution: ({ logs }) => {
          ...
          //
          // 50,000 ^ 2 * (10 ** -6) => 2500
          expect(swapInfoEvent.priceImpactUsd).eq(
            "2499999999999999953675116958000000"
          ); // ~2500
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        ...
        afterExecution: ({ logs }) => {
          const swapInfoEvent = getEventData(logs, "SwapInfo");
          expect(swapInfoEvent.priceImpactUsd).eq(
            "-199999999999999996247043298000000"
          ); // -200
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq("5960000000000000000"); // 5.96 ETH, 29,800
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(20_200, 6));

    expect(
      await dataStore.getUint(
        keys.swapImpactPoolAmountKey(ethUsdMarket.marketToken, wnt.address)
      )
    ).eq("40000000000000000");
    expect(
      await dataStore.getUint(
        keys.swapImpactPoolAmountKey(ethUsdMarket.marketToken, usdc.address)
      )
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        ...
        afterExecution: ({ logs }) => {
          const swapInfoEvent = getEventData(logs, "SwapInfo");
          expect(swapInfoEvent.priceImpactUsd).eq(
            "91839999999999998574976675000000"
          ); // 91.84 USD
  ...
  it("positive and negative impact fees", async () => {
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        ...
        afterExecution: ({ logs }) => {
          const swapInfoEvent = getEventData(logs, "SwapInfo");
          expect(swapInfoEvent.priceImpactUsd).eq(
            "-199500156249999996338625122000000"
          ); // -199.5
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq("5960099968750000000"); // 5.96 ETH, 29,800
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq("20237000157"); // 20,237.000157

    expect(
      await dataStore.getUint(
        keys.swapImpactPoolAmountKey(ethUsdMarket.marketToken, wnt.address)
      )
    ).eq("39900031250000000");
    expect(
      await dataStore.getUint(
        keys.swapImpactPoolAmountKey(ethUsdMarket.marketToken, usdc.address)
      )
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        ...
        afterExecution: ({ logs }) => {
          const swapInfoEvent = getEventData(logs, "SwapInfo");
          expect(swapInfoEvent.priceImpactUsd).eq(
            "91079461211532650093343730000000"
          ); // 91.0794612115 USD
