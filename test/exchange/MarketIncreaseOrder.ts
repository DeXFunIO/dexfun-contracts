...
describe("Exchange.MarketIncreaseOrder", () => {
  ...
  it("validates open interest", async () => {
    await dataStore.setUint(
      keys.maxOpenInterestKey(ethUsdMarket.marketToken, false),
  ...
  it("swaps tokens", async () => {
    ...
    expect(position0.position.numbers.collateralAmount).eq(
      expandDecimals(50000, 6)
    );
    expect(position0.position.numbers.borrowingFactor).eq(0);
    expect(position0.position.numbers.fundingFeeAmountPerSize).eq(0);
    expect(
      position0.position.numbers.longTokenClaimableFundingAmountPerSize
    ).eq(0);
    expect(
      position0.position.numbers.shortTokenClaimableFundingAmountPerSize
    ).eq(0);
