...
describe("Exchange.Withdrawal", () => {
  ...
  it("createWithdrawal", async () => {
    ...
    const withdrawal = await reader.getWithdrawal(
      dataStore.address,
      withdrawalKeys[0]
    );
  ...
  it("executeWithdrawal", async () => {
    ...
    const withdrawalKeys = await getWithdrawalKeys(dataStore, 0, 1);
    let withdrawal = await reader.getWithdrawal(
      dataStore.address,
      withdrawalKeys[0]
    );
    ...
    withdrawal = await reader.getWithdrawal(
      dataStore.address,
      withdrawalKeys[0]
    );
  ...
  it("executeWithdrawal, spot only market", async () => {
    ...
    expect(await getSupplyOf(ethUsdSpotOnlyMarket.marketToken)).eq(
    ...
    ).eq("2000000000000000"); // 0.002 ETH, ~10 USD
    expect(
      await getPoolAmount(
        dataStore,
        ethUsdSpotOnlyMarket.marketToken,
  ...
  it("price impact, fees", async () => {
    ...
    expect(await getMarketTokenPrice(fixture)).eq(
      "1000500500500500500500500500500"
    ); // 1.0005005
    ...
    expect(await getMarketTokenPrice(fixture)).eq(
      "2749275325325326000000000000000"
    ); // 2.74927532533

    expect(await getBalanceOf(ethUsdMarket.marketToken, user0.address)).eq(
      "10000000000000000000"
    ); // 10
    expect(await getSupplyOf(ethUsdMarket.marketToken)).eq(
      "10000000000000000000"
    ); // 10
    expect(await wnt.balanceOf(withdrawalHandler.address)).eq(0);
    expect(await usdc.balanceOf(withdrawalHandler.address)).eq(0);
    expect(await wnt.balanceOf(ethUsdMarket.marketToken)).eq(
      "11997500500500501"
    ); // 0.011997500500500501 ETH, ~60 USD
    expect(await usdc.balanceOf(ethUsdMarket.marketToken)).eq(0);
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq("5498550650650652"); // 0.005498550650650652, 27.5 USD
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(0);
    ...
    ).eq("1499774632447446"); // 0.001499774632447446
    expect(
      await getClaimableFeeAmount(
        dataStore,
        ethUsdMarket.marketToken,
    ...
    expect(await wnt.balanceOf(ethUsdMarket.marketToken)).eq(
      "6501699125175174"
    ); // 0.006501699125175174 ETH, ~32 USD
    expect(await usdc.balanceOf(ethUsdMarket.marketToken)).eq(0);
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq("1924492727728"); // 0.000001924492727728, 0.0096 USD
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(0);
  ...
  it("single token market", async () => {
    ...
    expect(
      await getBalanceOf(ethUsdSingleTokenMarket.marketToken, user0.address)
    ).eq(expandDecimals(20 * 1000, 18));
    ...
    expect(await usdc.balanceOf(ethUsdSingleTokenMarket.marketToken)).eq(
      expandDecimals(50 * 1000, 6)
    );
    ...
    expect(
      await getBalanceOf(ethUsdSingleTokenMarket.marketToken, user0.address)
    ).eq(expandDecimals(45 * 1000, 18));
    expect(await getSupplyOf(ethUsdSingleTokenMarket.marketToken)).eq(
      expandDecimals(45 * 1000, 18)
    );

    expect(await usdc.balanceOf(ethUsdSingleTokenMarket.marketToken)).eq(
      expandDecimals(45 * 1000, 6)
    );
