...
describe("Exchange.VirtualPositionPriceImpact", () => {
  ...
  beforeEach(async () => {
    ...
    await dataStore.setBytes32(
      keys.virtualTokenIdKey(wnt.address),
      ethUsdVirtualTokenId
    );
  ...
  it("uses virtual inventory for price impact", async () => {
    ...
    await dataStore.setUint(
      keys.positionImpactFactorKey(ethUsdcMarket.marketToken, true),
      decimalToFloat(5, 9)
    );
    await dataStore.setUint(
      keys.positionImpactFactorKey(ethUsdcMarket.marketToken, false),
      decimalToFloat(1, 8)
    );
    await dataStore.setUint(
      keys.positionImpactExponentFactorKey(ethUsdcMarket.marketToken),
      decimalToFloat(2, 0)
    );

    await dataStore.setUint(
      keys.positionImpactFactorKey(ethUsdtMarket.marketToken, true),
      decimalToFloat(5, 9)
    );
    await dataStore.setUint(
      keys.positionImpactFactorKey(ethUsdtMarket.marketToken, false),
      decimalToFloat(1, 8)
    );
    await dataStore.setUint(
      keys.positionImpactExponentFactorKey(ethUsdtMarket.marketToken),
      decimalToFloat(2, 0)
    );
    ...
    await usingResult(
      ...
      async ({ executeResult }) => {
        ...
        expect(positionIncreaseInfo.priceImpactUsd).eq(
