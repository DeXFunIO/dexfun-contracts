...
describe("Exchange.PositionFees", () => {
  ...
  beforeEach(async () => {
    ...
    ({ dataStore, ethUsdMarket, referralStorage, wnt, usdc } =
      fixture.contracts);
  ...
  it("position fees", async () => {
    ...
    expect(
      await dataStore.getUint(
        keys.collateralSumKey(ethUsdMarket.marketToken, usdc.address, false)
      )
    ).eq(0);

    expect(
      await dataStore.getUint(
        keys.poolAmountKey(ethUsdMarket.marketToken, wnt.address)
      )
    ).eq(expandDecimals(1000, 18));
    expect(
      await dataStore.getUint(
        keys.poolAmountKey(ethUsdMarket.marketToken, usdc.address)
      )
    ).eq(expandDecimals(500 * 1000, 6));
    ...
    ).eq("9980400000000000000"); // 9.9804 ETH, 49902 USD
    expect(
      await dataStore.getUint(
        keys.collateralSumKey(ethUsdMarket.marketToken, usdc.address, false)
      )
    ...
    ).eq("1000014400000000000000"); // 1000.0144 ETH => 5,000,072 USD
    ...
    ).eq("9952500000"); // 9952.5 USD

    expect(
      await dataStore.getUint(
        keys.poolAmountKey(ethUsdMarket.marketToken, wnt.address)
      )
    ).eq("1000014400000000000000");
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          expect(positionFeesCollectedEvent["collateralTokenPrice.min"]).eq(
            expandDecimals(5000, 12)
          );
          expect(positionFeesCollectedEvent["collateralTokenPrice.max"]).eq(
            expandDecimals(5000, 12)
          );
          expect(positionFeesCollectedEvent.tradeSizeUsd).eq(
            decimalToFloat(190 * 1000)
          );
          expect(positionFeesCollectedEvent.totalRebateFactor).eq(
            decimalToFloat(1, 1)
          ); // 10%
          expect(positionFeesCollectedEvent.traderDiscountFactor).eq(
            decimalToFloat(2, 1)
          ); // 20%
          expect(positionFeesCollectedEvent.totalRebateAmount).eq(
            "1900000000000000"
          ); // 0.0019 ETH => 9.5 USD
          expect(positionFeesCollectedEvent.traderDiscountAmount).eq(
            "380000000000000"
          ); // 0.00038 ETH => 1.9 USD
          expect(positionFeesCollectedEvent.affiliateRewardAmount).eq(
            "1520000000000000"
          ); // 0.00152 ETH => 7.6 USD
          expect(positionFeesCollectedEvent.fundingFeeAmount).closeTo(
            "1612803999999999",
            "10000000000"
          ); // 0.001612803999999999 ETH => 8.064019999 USD
          ...
          expect(positionFeesCollectedEvent.borrowingFeeAmount).closeTo(
            "1935344931032993",
            "10000000000"
          ); // 0.001935344931032993 ETH => 9.67671665528 USD
          ...
          ).closeTo("774137332422412", "10000000000"); // 0.000774137332422412 ETH => 3.87068666211 USD
          expect(positionFeesCollectedEvent.positionFeeFactor).eq(
            decimalToFloat(5, 4)
          );
          expect(positionFeesCollectedEvent.protocolFeeAmount).eq(
            "17100000000000000"
          ); // 0.0171 ETH => 85.5 USD
          expect(positionFeesCollectedEvent.positionFeeReceiverFactor).eq(
            decimalToFloat(2, 1)
          ); // 20%
          expect(positionFeesCollectedEvent.feeReceiverAmount).closeTo(
            "4194137332422412",
            "10000000000"
          ); // 0.004194137332422412 ETH => 20.9706866621 USD
          expect(positionFeesCollectedEvent.feeAmountForPool).closeTo(
            "14841205998633620",
            "10000000000"
          ); // 0.129800599863361968 ETH => 74.2060299932 USD
          expect(positionFeesCollectedEvent.positionFeeAmountForPool).eq(
            "13680000000000000"
          ); // 0.01368 ETH => 68.4 USD
          expect(positionFeesCollectedEvent.positionFeeAmount).eq(
            "19000000000000000"
          ); // 0.019 ETH => 95 USD
          expect(positionFeesCollectedEvent.totalCostAmount).closeTo(
            "22168147331056031",
            "10000000000"
          ); // 0.022168147331056031 ETH => 110.840736654 USD
          expect(
            positionFeesCollectedEvent.latestFundingFeeAmountPerSize
          ).closeTo("8064019999999995000000000", "10000000000000000000");
          expect(
            positionFeesCollectedEvent.latestLongTokenClaimableFundingAmountPerSize
          ).eq("0");
          expect(
            positionFeesCollectedEvent.latestShortTokenClaimableFundingAmountPerSize
          ).eq("0");
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          expect(positionFeesCollectedEvent.claimableLongTokenAmount).closeTo(
            "1612803999900000",
            "10000000000"
          ); // 0.0016128039999 ETH, 8.0640199995 USD
          expect(positionFeesCollectedEvent.claimableShortTokenAmount).eq("0");
          expect(positionFeesCollectedEvent.borrowingFeeAmount).closeTo(
            "4838114",
            "50"
          ...
          expect(
            positionFeesCollectedEvent.borrowingFeeAmountForFeeReceiver
          ).closeTo("1935245", "50"); // 1.935245 USD
          expect(positionFeesCollectedEvent.positionFeeFactor).eq(
            decimalToFloat(5, 4)
          );
          ...
          expect(
            positionFeesCollectedEvent.latestFundingFeeAmountPerSize
          ).closeTo("245454545455", "200000000000");
          expect(
            positionFeesCollectedEvent.latestLongTokenClaimableFundingAmountPerSize
          ).closeTo("16128039999999990000000000", "100000000000000000000");
          expect(
            positionFeesCollectedEvent.latestShortTokenClaimableFundingAmountPerSize
          ).eq(0);
          expect(positionFeesCollectedEvent.isIncrease).eq(false);
