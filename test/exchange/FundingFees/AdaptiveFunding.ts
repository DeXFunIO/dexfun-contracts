...
describe("Exchange.FundingFees.AdaptiveFunding", () => {
  ...
  beforeEach(async () => {
    ...
    ({ dataStore, reader, ethUsdMarket, ethUsdSingleTokenMarket, wnt, usdc } =
      fixture.contracts);
  ...
  it("adaptive funding", async () => {
    ...
    await dataStore.setUint(
      keys.thresholdForStableFundingKey(ethUsdMarket.marketToken),
      decimalToFloat(5, 2)
    ); // 5%
    await dataStore.setUint(
      keys.thresholdForDecreaseFundingKey(ethUsdMarket.marketToken),
      decimalToFloat(3, 2)
    ); // 3%
    await dataStore.setUint(
    ...
    await dataStore.setUint(
      keys.fundingDecreaseFactorPerSecondKey(ethUsdMarket.marketToken),
      decimalToFloat(2, 8)
    ); // 0.000002%
    await dataStore.setUint(
      keys.minFundingFactorPerSecondKey(ethUsdMarket.marketToken),
      0
    );
    await dataStore.setUint(
      keys.maxFundingFactorPerSecondKey(ethUsdMarket.marketToken),
      decimalToFloat(1)
    );
    ...
    await usingResult(
      ...
      (marketInfo) => {
        expect(marketInfo.nextFunding.fundingFactorPerSecond).eq(ethers.BigNumber.from(0));
    ...
    await usingResult(
      ...
      (marketInfo) => {
        ...
        expect(marketInfo.nextFunding.fundingFactorPerSecond).eq(
          "36000000000000000000000000"
        ); // 0.0036%
        expect(marketInfo.nextFunding.nextSavedFundingFactorPerSecond).eq(
          "36000000000000000000000000"
        ); // 0.0036%
      }
    );
