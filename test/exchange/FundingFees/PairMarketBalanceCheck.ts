...
describe("Exchange.FundingFees.PairMarketBalanceCheck", () => {
  ...
  it("funding fees after funding switches sides", async () => {
    ...
    ).eq(decimalToFloat(300_000));
    expect(
      await dataStore.getUint(
        keys.openInterestKey(ethUsdMarket.marketToken, usdc.address, true)
      )
    ).eq(decimalToFloat(100_000));
    expect(
      await dataStore.getUint(
    ...
    expect(
      await dataStore.getUint(
        keys.openInterestKey(ethUsdMarket.marketToken, usdc.address, false)
      )
    ).eq(decimalToFloat(100_000));
    ...
    ).eq(expandDecimals(20, 18));
    expect(
      await dataStore.getUint(
        keys.collateralSumKey(ethUsdMarket.marketToken, usdc.address, true)
      )
    ).eq(expandDecimals(50_000, 6));
    expect(
      await dataStore.getUint(
        keys.collateralSumKey(ethUsdMarket.marketToken, wnt.address, false)
      )
    ).eq(expandDecimals(5, 18));
    expect(
      await dataStore.getUint(
        keys.collateralSumKey(ethUsdMarket.marketToken, usdc.address, false)
      )
    ).eq(expandDecimals(25_000, 6));
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: async ({ logs }) => {
          ...
          expect(feeInfo.fundingFeeAmount).closeTo(
            "806406800000000000",
            "10000000000000"
          ); // 0.8064068 ETH, 4032.034 USD
          expect(feeInfo.collateralToken).eq(wnt.address);
          const claimableFundingData = getEventDataArray(
            logs,
    ...
    ).eq(decimalToFloat(300_001));

    await time.increase(28 * 24 * 60 * 60);

    expect(await wnt.balanceOf(user0.address)).closeTo(
      "8387186400000000000",
      "10000000000000"
    );
    ...
    expect(await usdc.balanceOf(user0.address)).closeTo(
      "33871772052",
      "100000"
    ); // 33,871.772054 USDC

    expect(await wnt.balanceOf(user1.address)).closeTo(
      "9193593200000000000",
      "100000000000000"
    ); // 9.1935932 ETH
    expect(await usdc.balanceOf(user1.address)).eq(0);
    ...
    expect(await wnt.balanceOf(user2.address)).eq(0);
    expect(await usdc.balanceOf(user2.address)).closeTo(
      "45967966000",
      "100000"
    ); // 45,967.966 USDC

    expect(await wnt.balanceOf(user3.address)).closeTo(
      "3387175205252222237",
      "10000000000000"
    ); // 3.387175205252222237 ETH
    expect(await usdc.balanceOf(user3.address)).eq(0);
    ...
    expect(await wnt.balanceOf(user1.address)).closeTo(
      "10000000565295075039",
      "10000000000000"
    ); // 10.000000565295075039 ETH, 50,000.0028265 USD
    expect(await usdc.balanceOf(user1.address)).closeTo(
      "12096150798",
      "1000000"
    ); // 12,096.150798 USDC
    ...
    expect(await wnt.balanceOf(user2.address)).closeTo(
      "806417429452702722",
      "10000000000000"
    ); // 0.806417429452702722 ETH, 4032.08714726 USD
    expect(await usdc.balanceOf(user2.address)).closeTo(
      "58064247762",
      "1000000"
    ); // 58,064.247762 USDC
    ...
    expect(await wnt.balanceOf(user3.address)).closeTo(
      "4596792605252222236",
      "10000000000000"
    ); // 4.596792605252222236 ETH, 22,983.9630263 USD
    expect(await usdc.balanceOf(user3.address)).closeTo(
      "2016028999",
      "1000000"
    ); // 2016.028999
    ...
    expect(await wnt.balanceOf(user4.address)).closeTo(
      "1209602999999999999",
      "10000000000000"
    ); // 1.209602999999999999 ETH, 6048.015 USD
    expect(await usdc.balanceOf(user4.address)).closeTo(
      "18951800383",
      "1000000"
    ); // 18,951.800383 USDC
