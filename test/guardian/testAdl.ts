...
describe("Guardian.AdlOrder", () => {
  ...
  beforeEach(async () => {
    ...
    ({ roleStore, dataStore, ethUsdMarket, solUsdMarket, wnt, usdc, reader } =
      fixture.contracts);
  ...
  it("ADL cannot execute if block numbers for price are before latestAdlBlock", async () => {
    ...
    const maxPnlFactorForAdlKey = keys.maxPnlFactorKey(
      keys.MAX_PNL_FACTOR_FOR_ADL,
      ethUsdMarket.marketToken,
      true
    );
    const minPnlFactorAfterAdlKey = keys.minPnlFactorAfterAdl(
      ethUsdMarket.marketToken,
      true
    );
    ...
    expect(await getIsAdlEnabled(dataStore, ethUsdMarket.marketToken, true)).eq(
      true
    );
  ...
  it("ADL cannot execute when PnL to pool ratio not exceeded", async () => {
    ...
    const maxPnlFactorKey = keys.maxPnlFactorKey(
      keys.MAX_PNL_FACTOR,
    ...
    expect(await getIsAdlEnabled(dataStore, solUsdMarket.marketToken, true)).be
      .false;
