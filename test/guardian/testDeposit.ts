...
describe("Guardian.Deposit", () => {
  ...
  beforeEach(async () => {
    ...
    ({ reader, dataStore, ethUsdMarket, ethUsdSpotOnlyMarket, wnt, usdc } =
      fixture.contracts);
  ...
  it("Deposit long token with swap path", async () => {
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq(expandDecimals(10, 18)); // $50,000
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(50 * 1000, 6)); // $50,000
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(100 * 1000, 6)); // $100,000
    ...
    ).eq(expandDecimals(20, 18)); // $100,000
    expect(
      await getPoolAmount(
        dataStore,
        ethUsdSpotOnlyMarket.marketToken,
  ...
  it("Deposit short token with swap path", async () => {
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(150 * 1000, 6)); // $150,000
  ...
  it("Deposit both long token and short token with a swap path for both", async () => {
    ...
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
    ).eq(expandDecimals(5, 18)); // $25,000
    expect(
      await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
    ).eq(expandDecimals(125 * 1000, 6)); // $125,000
  ...
  it("Deposit both long token and short token with a swap path for long", async () => {
    ...
    ).eq(expandDecimals(15, 18)); // $75,000
    expect(
      await getPoolAmount(
        dataStore,
        ethUsdSpotOnlyMarket.marketToken,
    ...
    ).eq(expandDecimals(25 * 1000, 6)); // $25,000
