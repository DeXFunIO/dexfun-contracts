...
describe("Guardian.OIReserve", () => {
  ...
  it("test OI Reserve", async () => {
    ...
    const oiReserveKey = keys.openInterestReserveFactorKey(
    ...
    // Read normal reserve
    const normalReserveFactorKey = keys.reserveFactorKey(
      ethUsdMarket.marketToken,
      true
    );
    ...
    expect(await dataStore.getUint(normalReserveFactorKey)).to.eq(
      expandDecimals(6, 29)
    );
