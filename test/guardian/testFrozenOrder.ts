...
describe("Guardian.FrozenOrder", () => {
  ...
  beforeEach(async () => {
    ...
    ({ reader, dataStore, ethUsdMarket, wnt, usdc, exchangeRouter } =
      fixture.contracts);
  ...
  it("User can unfreeze a frozen limit increase order by calling updateOrder", async () => {
    ...
    const orderPreUpdate = await reader.getOrder(
      dataStore.address,
      orderKeys[0]
    );
    ...
    const orderPostUpdate = await reader.getOrder(
      dataStore.address,
      orderKeys[0]
    );
    ...
    expect(orderPostUpdate.numbers.acceptablePrice).eq(
      expandDecimals(5200, 12)
    );
    expect(orderPostUpdate.numbers.triggerPrice).eq(expandDecimals(5200, 12));
    expect(orderPostUpdate.numbers.minOutputAmount).eq(
