...
describe("Guardian.Withdrawal", () => {
  ...
  it("Deposit one token but get both back on withdraw", async () => {
    ...
    ).eq(expandDecimals(50000, 18));
    expect(
      await getBalanceOf(ethUsdSpotOnlyMarket.marketToken, user1.address)
    ).eq(expandDecimals(100000, 18)); // Twice as much deposited
    expect(await getSupplyOf(ethUsdSpotOnlyMarket.marketToken)).eq(
      expandDecimals(150000, 18)
    );
    expect(
      await getPoolAmount(
        dataStore,
    ...
    ).eq(expandDecimals(50_000 - 10_000, 6));
    // Expect market token supply is properly decremented
    expect(await getSupplyOf(ethUsdSpotOnlyMarket.marketToken)).eq(
      expandDecimals(120000, 18)
    );
    expect(
      await getBalanceOf(ethUsdSpotOnlyMarket.marketToken, user0.address)
    ).eq(expandDecimals(20000, 18));
    ...
    expect(await wnt.balanceOf(user0.address)).eq(
      BigNumber.from("2666666666666666666").add(wntBalAfterWithdraw1)
    );
    expect(await usdc.balanceOf(user0.address)).eq(
      BigNumber.from("6666666666").add(usdcBalAfterWithdraw1)
    );
    ...
    ).eq("13333333333333333334"); // 2/3 * 20 ETH
    expect(
      await getPoolAmount(
        dataStore,
        ethUsdSpotOnlyMarket.marketToken,
    ...
    ).eq("33333333334"); // 2/3 * 50,000 USDC
    // Expect market token supply is properly decremented
    expect(await getSupplyOf(ethUsdSpotOnlyMarket.marketToken)).eq(
