...
describe("Guardian.Swap", () => {
  ...
  describe("Deposit", () => {
    ...
    it("multiswaps through a regular market during deposit", async () => {
      ...
      expect(
        await getPoolAmount(dataStore, btcUsdMarket.marketToken, wbtc.address)
      ).eq(expandDecimals(102, 7));
      expect(
        await getPoolAmount(dataStore, btcUsdMarket.marketToken, usdc.address)
      ).eq(expandDecimals(490_000, 6));

      expect(
        await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
      ).eq(expandDecimals(12, 18));
      expect(
        await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
      ).eq(expandDecimals(60_000, 6));

      expect(
        await getPoolAmount(dataStore, ethUsdtMarket.marketToken, wnt.address)
      ).eq(expandDecimals(8, 18));
      expect(
        await getPoolAmount(dataStore, ethUsdtMarket.marketToken, usdt.address)
      ).eq(expandDecimals(60_000, 6));
    ...
    it("multiswaps through a spot only market during deposit", async () => {
      ...
      ).eq(expandDecimals(9 * 5000, 6));

      expect(
        await getPoolAmount(dataStore, ethUsdtMarket.marketToken, wnt.address)
      ).eq(expandDecimals(9, 18));
      expect(
        await getPoolAmount(dataStore, ethUsdtMarket.marketToken, usdt.address)
      ).eq(expandDecimals(11 * 5000, 6));
    ...
    it("multiswaps through a regular market during order execution", async () => {
      ...
      await executeOrder(fixture, {
        ...
        afterExecution: ({ logs }) => {
          ...
          expect(positionIncreaseEvent.sizeDeltaUsd).eq(
            "20000000000000000000000000000000000"
          ); // 20,000 * 1e30
          expect(positionIncreaseEvent.sizeDeltaInTokens).eq(
            "4004004004004004004"
          ); // 4x leverage
          // (5000 * 1e18) / (4995) = 1001001001001001001 - we put $5000 worth of BTC and swap to $5000  worth of WNT, which is at $4995
          expect(positionIncreaseEvent.collateralAmount).eq(
            "1001001001001001001"
          );
        },
      });
      ...
      expect(
        await getPoolAmount(dataStore, btcUsdMarket.marketToken, wbtc.address)
      ).eq(expandDecimals(101, 7)); // 10.1 BTC
      expect(
        await getPoolAmount(dataStore, btcUsdMarket.marketToken, usdc.address)
      ).eq(expandDecimals(9.9 * 50000, 6));

      // 1001001001001001001 of the pool is reserved by the increase order position
      expect(
        await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)
      ).eq("8998998998998998999");
      expect(
        await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)
      ).eq(expandDecimals(55_000, 6));
