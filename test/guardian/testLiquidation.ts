...
describe("Guardian.Liquidation", () => {
  ...
  it("User can get liquidated if they invalidate the minCollateralUsdForLeverage", async () => {
    ...
    expect(
      (await provider.getBalance(user1.address)).sub(etherBalInitial)
    ).to.eq(initialWNTBalance);
  });
  ...
  it("User can get liquidated if they invalidate the minCollateralUsdForLeverage", async () => {
    ...
    expect(
      (await provider.getBalance(user1.address)).sub(etherBalInitial)
    ).to.eq(ethers.utils.parseEther(".88"));
  });
  ...
  it("Liquidate short position because index token price increases", async () => {
    ...
    expect(
      (await provider.getBalance(user1.address)).sub(etherBalInitial)
    ).to.eq(ethers.utils.parseEther(".898"));
  });
  ...
  it("Liquidate someone with more losses than collateral", async () => {
    ...
    const poolBalBefore = await getPoolAmount(
      dataStore,
    ...
    expect(
      (
        await getPoolAmount(dataStore, solUsdMarket.marketToken, wnt.address)
      ).sub(poolBalBefore)
    ).to.eq(initialWNTBalance);
    expect(
      (await ethers.provider.getBalance(user1.address)).sub(userNativeBalBefore)
    ).to.eq(0);
  ...
  it("Liquidate someone in profit & they receive their native tokens", async () => {
    ...
    await executeLiquidation(fixture, {
      ...
      precisions: [8, 18, 8],
      minPrices: [
        expandDecimals(4000, 4),
        expandDecimals(1, 6),
        expandDecimals(1501, 2),
      ], // SOL goes to $15.01 & ETH goes to $4000
      maxPrices: [
        expandDecimals(4000, 4),
        expandDecimals(1, 6),
        expandDecimals(1501, 2),
    ...
    expect(
      (await ethers.provider.getBalance(user1.address)).sub(userNativeBalBefore)
    ).to.eq(pnlAmount.add(collateralAmount));
    expect((await usdc.balanceOf(user1.address)).sub(userUSDCBalBefore)).to.eq(
      0
    );
  });
  ...
  it("Liquidate someone in profit & their pnl is swapped to collateral tokens as output", async () => {
    ...
    await executeLiquidation(fixture, {
      ...
      minPrices: [
        ...
        expandDecimals(8, 5),
        expandDecimals(1501, 2),
      ], // SOL goes to $15.01 & USDC goes to $0.80
      maxPrices: [
        expandDecimals(4000, 4),
        expandDecimals(8, 5),
        expandDecimals(1501, 2),
