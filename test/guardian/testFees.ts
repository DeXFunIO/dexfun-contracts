...
describe("Guardian.Fees", () => {
  ...
  it("Positive & negative impact fees for positions", async () => {
    ...
    expect(negativeImpactPositionFeeFactor).to.gt(
      positiveImpactPositionFeeFactor
    );
    ...
    [marketTokenPrice, poolValueInfo] = await getMarketTokenPriceWithPoolValue(
      fixture,
      {
    ...
    expect(position.numbers.collateralAmount).to.eq(
      expandDecimals(25_000, 6).sub(expandDecimals(50, 6))
    );
    ...
    expect(poolValueInfo.poolValue).to.eq(
      expandDecimals(10_000_000, 30).add(expandDecimals(50, 30))
    ); // 10M + $50 of fees

    let feeAmountCollected = expandDecimals(50, 6);

    expect(poolValueInfo.shortTokenAmount).to.eq(
      expandDecimals(5_000_000, 6).add(feeAmountCollected)
    );
    expect(poolValueInfo.longTokenAmount).to.eq(expandDecimals(1_000, 18));
    ...
    const positionKey2 = getPositionKey(
      user0.address,
      ethUsdMarket.marketToken,
      usdc.address,
      false
    ...
    expect(position2.numbers.collateralAmount).to.eq(
      expandDecimals(25_000, 6).sub(expandDecimals(25, 6))
    );
    ...
    expect(poolPnl).to.closeTo(
      expandDecimals(187425, 27),
      expandDecimals(1, 17)
    ); // $74.95 + $112.475 = $187.425 with negligible imprecision
    ...
    expect(poolPnl).to.closeTo(
      expandDecimals(212425, 27).mul(-1),
      expandDecimals(1, 17)
    ); // -$124.95 - $87.475 = -$212.425 with imprecision
    ...
    expect(poolValueInfo.poolValue).to.eq(
      expandDecimals(10_000_000, 30).add(expandDecimals(75, 30))
    ); // 10M + $75 of fees
    ...
    await dataStore.setUint(
      keys.minCollateralFactorForOpenInterestMultiplierKey(
        ethUsdMarket.marketToken,
        false
      ),
      decimalToFloat(6, 6)
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          const autoAdjustCollateralEvent = getEventData(
            logs,
            "OrderCollateralDeltaAmountAutoUpdated"
          );

          expect(autoAdjustCollateralEvent.collateralDeltaAmount).to.eq(
            expandDecimals(24_975, 6)
          );
    ...
    expect(poolPnl).to.closeTo(
      expandDecimals(1311875, 26),
      expandDecimals(1, 17)
    ); // $74.95 + $56.2375 = $131.1875 with negligible imprecision
    ...
    expect(poolPnl).to.closeTo(
      expandDecimals(1686875, 26).mul(-1),
      expandDecimals(1, 17)
    ); // -$124.95 - $43.7375 = -$168.6875 with negligible imprecision
    ...
    expect(user0UsdcBalAfter.sub(user0UsdcBalBefore)).to.eq(
      expandDecimals(25_000, 6)
        .sub(expandDecimals(100, 6))
        .sub(expandDecimals(3125, 3))
        .sub(expandDecimals(25, 6))
    );
    ...
    expect(poolPnl).to.closeTo(
      expandDecimals(562375, 26),
      expandDecimals(1, 17)
    ); // $56.2375 with negligible imprecision
    ...
    expect(poolPnl).to.closeTo(
      expandDecimals(437375, 26).mul(-1),
      expandDecimals(1, 17)
    ); // $43.7375 with negligible imprecision
    ...
    expect(poolValueInfo.poolValue).to.eq(
      expandDecimals(10_000_000, 30)
        .add(expandDecimals(150, 30))
        .add("1000000000000000000000000")
    ); // 10M + $150 of fees & imprecision

    feeAmountCollected = feeAmountCollected.add(expandDecimals(50, 6));
    priceImpactAmountPaidToPool = priceImpactAmountPaidToPool.add(
      expandDecimals(3125, 3)
    );
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        afterExecution: ({ logs }) => {
          ...
          expect(positionDecreasedEvent.collateralDeltaAmount).to.eq(
            expandDecimals(14_500, 6)
          );
          expect(positionDecreasedEvent.collateralAmount).to.eq(
            expandDecimals(10_450, 6).sub(1)
          ); // 1 wei imprecision
        },
      },
    });
    ...
    // extra 100000000000000000 is from roundUp division on applying an amount paid for negative PI to the pool
    // Vs. using round down division for deducting positive PnL from the pool.
    const marketTokenPriceBefore = BigNumber.from(
      "1000015000000100000000000000000"
    );
    ...
    await executeLiquidation(fixture, {
      ...
      afterExecution: ({ logs }) => {
        ...
        // 25,000^2 * 5e21 / 1e30 = $3.125
        expect(positionDecreasedEvent.priceImpactUsd).to.closeTo(
          expandDecimals(3125, 27),
          expandDecimals(1, 17)
        ); // ~$3.125 in positive impact
      },
    });
    ...
    priceImpactAmountPaidToPool = priceImpactAmountPaidToPool.sub(
      expandDecimals(3125, 3)
    );
    ...
    const depositedValue = poolValueInfo.shortTokenAmount
      .mul(expandDecimals(1, 24))
      .add(expandDecimals(5_000_000, 30));

    expect(poolValueInfo.poolValue).to.eq(
      depositedValue.sub(impactPoolAmount.add(1).mul(expandDecimals(5000, 12)))
    );
