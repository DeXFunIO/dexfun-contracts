...
describe("Guardian.GasEstimation", () => {
  ...
  beforeEach(async () => {
    ...
    ({ roleStore, dataStore, ethUsdMarket, solUsdMarket, wnt, usdc, config } =
      fixture.contracts);
  ...
  it("Estimates gas properly for deposits with swapPaths", async () => {
    ...
    await config
      .connect(wallet)
      .setUint(keys.ESTIMATED_GAS_FEE_BASE_AMOUNT, "0x", 100_000);
    await config
      .connect(wallet)
      .setUint(keys.DEPOSIT_GAS_LIMIT, encodeData(["bool"], [false]), 300_000);
    await config
      .connect(wallet)
      .setUint(
        keys.ESTIMATED_GAS_FEE_MULTIPLIER_FACTOR,
        "0x",
        expandDecimals(15, 29)
      ); // 1.5x
    await config
      .connect(wallet)
      .setUint(keys.SINGLE_SWAP_GAS_LIMIT, "0x", 25_000);
  ...
  it("Estimates gas properly for withdrawals with swapPaths", async () => {
    ...
    await config
      .connect(wallet)
      .setUint(keys.withdrawalGasLimitKey(), "0x", 300_000);
    await config
      .connect(wallet)
      .setUint(
        keys.ESTIMATED_GAS_FEE_MULTIPLIER_FACTOR,
  ...
  it("Estimates gas properly for increase orders with swaps", async () => {
    ...
    await config
      .connect(wallet)
      .setUint(keys.INCREASE_ORDER_GAS_LIMIT, "0x", 300_000);
  ...
  it("Estimates gas properly for decrease orders with swaps", async () => {
    ...
    await config
      .connect(wallet)
      .setUint(keys.DECREASE_ORDER_GAS_LIMIT, "0x", 300_000);
  ...
  it("Estimates gas properly for swap orders", async () => {
    ...
    await config
      .connect(wallet)
      .setUint(keys.SWAP_ORDER_GAS_LIMIT, "0x", 300_000);
