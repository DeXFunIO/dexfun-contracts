...
describe("Guardian.HomogenousMarkets", () => {
  ...
  it("Long token == short token, deposit long token", async () => {
    ...
    expect(ethUsdSingleTokenMarket.longToken).to.eq(
      ethUsdSingleTokenMarket.shortToken
    );
    ...
    expect(
      await getBalanceOf(ethUsdSingleTokenMarket.marketToken, user1.address)
    ).eq(expandDecimals(500000, 18));
