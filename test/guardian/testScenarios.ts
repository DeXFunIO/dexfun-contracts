...
describe("Guardian.Scenarios", () => {
  ...
  it("Open a long & short position, price goes up", async () => {
    ...
    const longPositionKey = await getPositionKey(
      user1.address,
  ...
  it("Open a long & short position, price goes down", async () => {
    ...
    expect(wntBalAfter.sub(wntBalBefore)).to.eq(0);
    expect(usdcBalAfter.sub(usdcBalBefore)).to.eq(
      collateralAmount.sub(expandDecimals(20_000, 6))
    );
    ...
    expect(wntBalAfter.sub(wntBalBefore)).to.eq(0);
    expect(usdcBalAfter.sub(usdcBalBefore)).to.eq(
      collateralAmount.add(expandDecimals(20_000, 6))
    );
