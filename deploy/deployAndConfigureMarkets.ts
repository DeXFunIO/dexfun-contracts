...
const func = async ({ deployments, getNamedAccounts, dFun, network }: HardhatRuntimeEnvironment) => {
  const runForNetworks = ["localhost", "hardhat", "neoX"];
  if (!runForNetworks.includes(network.name)) {
    return;
  }

  const { execute, get, read, log } = deployments;
  const generalConfig = await dFun.getGeneral();
  ...
  async function setMinCollateralFactor(marketToken: string, minCollateralFactor: number | BigNumber) {
  ...
  async function setMaxPoolAmount(marketToken: string, token: string, maxPoolAmount: number | BigNumber) {
  ...
  if (network.name === "hardhat" || network.name === 'localhost') {
    // To disable automining
    await network.provider.send("evm_setAutomine", [false]);

    // To enable interval mining with a 5 second interval
    await network.provider.send("evm_setIntervalMining", [10000]);
  }

};

func.skip = async ({ dFun, network }) => {
...
func.tags = ["Markets"];
func.dependencies = ["MarketFactory", "Tokens", "DataStore", "FunTokens"];
export default func;
