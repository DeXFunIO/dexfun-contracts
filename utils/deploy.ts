...
export function createDeployFunction({
  ...
  const func = async ({ getNamedAccounts, deployments, dFun, network }: HardhatRuntimeEnvironment) => {
    ...
    let deployArgs = [];
    if (getDeployArgs) {
      deployArgs = await getDeployArgs({ dependencyContracts, network, dFun, get, getNamedAccounts });
    }
    ...
    if (afterDeploy) {
      await afterDeploy({ deployedContract, deployer, getNamedAccounts, deployments, dFun, network });
    }
