...
export async function deployFixture() {
  ...
  const wnt = await hre.ethers.getContract("WGAS");
  await wnt.deposit({ value: ethers.utils.parseUnits("50", 18) });

  const wbtc = await hre.ethers.getContract("WBTC");
  const sol = {
    address: getSyntheticTokenAddress(network.config.chainId, "SOL"),
  };

  const usdc = await hre.ethers.getContract("USDC");
  const usdt = await hre.ethers.getContract("USDT");

  const oracleSalt = hashData(
    ["uint256", "string"],
    [chainId, "xget-oracle-v1"]
  );
  ...
  const decreasePositionUtils = await hre.ethers.getContract(
    "DecreasePositionUtils"
  );
  const increaseOrderUtils = await hre.ethers.getContract("IncreaseOrderUtils");
  const increasePositionUtils = await hre.ethers.getContract(
  ...
  const ethUsdMarket = await reader.getMarket(
    dataStore.address,
    ethUsdMarketAddress
  );
  ...
  const ethUsdtMarket = await reader.getMarket(
    dataStore.address,
    ethUsdtMarketAddress
  );
  ...
  const ethUsdSpotOnlyMarket = await reader.getMarket(
    dataStore.address,
    ethUsdSpotOnlyMarketAddress
  );
  ...
  const ethUsdSingleTokenMarket = await reader.getMarket(
    dataStore.address,
    ethUsdSingleTokenMarketAddress
  );
  ...
  const btcUsdMarket = await reader.getMarket(
    dataStore.address,
    btcUsdMarketAddress
  );
  ...
  const solUsdMarket = await reader.getMarket(
    dataStore.address,
    solUsdMarketAddress
  );
  ...
  return {
    ...
    contracts: {
      ...
      baseOrderUtils,
      orderUtils,
      liquidationHandler: liquidationHandler.connect(user0),
      adlHandler,
      router,
      exchangeRouter,
      subaccountRouter,
