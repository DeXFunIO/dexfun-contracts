...
export async function getMarketTokenPriceWithPoolValue(
  fixture,
  overrides: any = {}
) {
  ...
  const pnlFactorType =
    overrides.pnlFactorType || keys.MAX_PNL_FACTOR_FOR_TRADERS;
...
export function getMarketTokenAddress(
  ...
  const salt = hashData(
    ...
    ["DFUN_MARKET", indexToken, longToken, shortToken, marketType]
  );
...
export function createMarketConfigByKey({ marketConfigs, tokens }) {
  ...
  for (const marketConfig of marketConfigs) {
    const [indexToken, longToken, shortToken] = getMarketTokenAddresses(
      marketConfig,
      tokens
    );
...
export async function getOnchainMarkets(
  ...
  >
> {
  const onchainMarkets = await read(
    "Reader",
    "getMarkets",
    dataStoreAddress,
    0,
