...
// then run:
//
// REALTIME_FEED_CLIENT_ID=<clien-id> REALTIME_FEED_CLIENT_SECRET=<client-secret> ACCOUNT_KEY_FILE=key-file.json npx hardhat run --network neoXT4 scripts/createOrder.ts
//
...
async function main() {
  ...
  const ETH_USD_MARKET = "0x15b0aac0653972A175B910e00203FB366355b86c";

  // list of tokens can be found in config/tokens.ts
  let USDC = "0x3321Fd36aEaB0d5CdfD26f4A3A93E2D2aAcCB99f";

  if(['localhost', 'harhdat'].includes(hre.network.name)) {
