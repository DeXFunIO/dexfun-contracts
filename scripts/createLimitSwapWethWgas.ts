...
async function main() {
  ...
  console.log("ethusdc market %s", wethUsdMarketAddress);
  console.log("wgasusdc market %s", wgasUsdMarketAddress);
  ...
  const params: IBaseOrderUtils.CreateOrderParamsStruct = {
    addresses: {
      ...
      swapPath: [wethUsdMarketAddress, wgasUsdMarketAddress],
    },
    numbers: {
      sizeDeltaUsd: 0,
      triggerPrice: BigNumber.from(ethers.utils.parseUnits('3.74', 8)),
      acceptablePrice: 0,
      executionFee,
      callbackGasLimit: 0,
      minOutputAmount: 0,
    ...
    orderType: 1, // MarketSwap
    isLong: false, // not relevant for swaps
    shouldUnwrapNativeToken: false, // no t relevant for swaps
    decreasePositionSwapType: 0, // not relevant for swaps
