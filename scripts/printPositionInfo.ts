...
function getNeoXT4Values() {
  return {
    oracleApi: "https://dexfun-api-staging.vercel.app/api/",
    referralStorageAddress: ""
  };
}

function getNeoXValues() {
  return {
    oracleApi: "",
    referralStorageAddress: ""
  };
...
function getValues(): {
  ...
  if (hre.network.name === "neoXT4") {
    return getNeoXT4Values();
  } else if (hre.network.name === "neoX") {
    return getNeoXValues();
  }
...
async function main() {
  ...
  const tokens = (await (hre as any).dFun.getTokens()) as TokensConfig;
