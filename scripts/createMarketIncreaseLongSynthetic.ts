...
async function getValues(): Promise<{
  ...
  if (hre.network.name === "neoXT4") {
    ...
    return {
      ...
      syntheticTokenAddress: tokens.SOL.address
    };
  }
...
async function main() {
  ...
  const { wnt, syntheticTokenAddress } = await getValues();
  ...
  const longTokenAmount = expandDecimals(10, 18); // 0.01 weth
  ...
  console.log("Synthetic token address", syntheticTokenAddress);
  const wethUsdMarketAddress = await getMarketTokenAddress(
    syntheticTokenAddress ? syntheticTokenAddress : weth.address,
    weth.address,
    usdc.address,
    DEFAULT_MARKET_TYPE,
